...
Rs.ux.Carousel = Rs.extend(Rs.util.Observable, {
    interval: 3,
    ...
    constructor: function(elId, config) {
        ...
        Rs.ux.Carousel.superclass.constructor.call(this, config);
    ...
    initEvents: function() {
        this.els.navPrev.on('click', function(ev) {
            ...
            target.blur();            
            if(Rs.fly(target).hasClass('ux-carousel-nav-disabled')) return;
            this.prev();
        }, this);
    ...
    play: function() {
        if(!this.playing) {
            ...
            this.playTaskBuffer = this.playTaskBuffer || new Rs.util.DelayedTask(function() {
                Rs.TaskMgr.start(this.playTask);
            }, this);
    ...
    add: function(el, refresh) {
        var item = Rs.fly(el).appendTo(this.els.slidesWrap).wrap({cls: 'ux-carousel-slide'});
        item.setWidth(this.slideWidth + 'px').setHeight(this.slideHeight + 'px');
        this.slides.push(item);                        
        if(refresh) {
            this.refresh();

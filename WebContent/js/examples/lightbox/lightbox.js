...
Rs.ux.Lightbox = (function(){
    ...
    return {
        ...
        init: function() {
            ...
            if(!initialized) {
                Rs.apply(this, Rs.util.Observable.prototype);
                Rs.util.Observable.constructor.call(this);
        ...
        initMarkup: function() {
            els.shim = Rs.DomHelper.append(document.body, {
                tag: 'iframe',
                id: 'ux-lightbox-shim'
            }, true);
            els.overlay = Rs.DomHelper.append(document.body, {
                id: 'ux-lightbox-overlay'
            }, true);
            
            var lightboxTpl = new Rs.Template(this.getTemplate());
            ...
            Rs.each(ids, function(id){
                els[id] = Rs.get('ux-lightbox-' + id);
            });

            Rs.each([els.overlay, els.lightbox, els.shim], function(el){
                el.setVisibilityMode(Rs.Element.DISPLAY)
                el.hide();
            });
        ...
        register: function(sel, group) {
            if(selectors.indexOf(sel) === -1) {
                ...
                Rs.fly(document).on('click', function(ev){
            ...
            els.overlay.fadeIn({
                ...
                callback: function() {
                    ...
                    if(!group) {
                        ...
                        var setItems = Rs.query(sel);
                        Rs.each(setItems, function(item) {
                            if(item.href) {
                    ...
                    var pageScroll = Rs.fly(document).getScroll();

                    var lightboxTop = pageScroll.top + (Rs.lib.Dom.getViewportHeight() / 10);
        ...
        enableKeyNav: function() {
            Rs.fly(document).on('keydown', this.keyNavAction, this);
        },

        disableKeyNav: function() {
            Rs.fly(document).un('keydown', this.keyNavAction, this);
        },
        ...
        getViewSize: function() {
            return [Rs.lib.Dom.getViewWidth(), Rs.lib.Dom.getViewHeight()];
        }
    }
})();

Rs.onReady(Rs.ux.Lightbox.init, Rs.ux.Lightbox);
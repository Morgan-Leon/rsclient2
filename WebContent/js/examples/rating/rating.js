...
Rs.ux.Rating = Rs.extend(Rs.util.Observable, {
    ...
    constructor : function(element, config) {
        ...
        Rs.ux.Rating.superclass.constructor.call(this);
    ...
    init : function() {
        ...
        if(this.canReset) {
            ...
            this.resetEl.visibilityMode = Rs.Element.DISPLAY;
            
            this.resetEl.hover(function() {
                Rs.fly(this).addClass('ux-rating-reset-hover');
            }, function() {
                Rs.fly(this).removeClass('ux-rating-reset-hover');
            });
    ...
    fillTo : function(index, hover) {
        if (index != -1) {
            ...
            Rs.each(this.stars.slice(0, index+1), function() {
                Rs.fly(this).removeClass(removeClass).addClass(addClass);
            });

			// And then remove the same class from all the stars after this one
            Rs.each(this.stars.slice(index+1), function() {
                Rs.fly(this).removeClass([removeClass, addClass]);
            });
        }
        else {
            this.fillNone();

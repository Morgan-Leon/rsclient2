Rs.define(
    ...
    'InfoArea', {
            ...
            tbar : [{
                ...
                menu:{
                    items:[{
                        ...
                        scope:this,
                        handler : function(){
                            if(!this.activeNode || this.activeNode.level=="5"){
                                alert("不能添加子项");
                                return;
                            }
                            var addWindow = Rs.getAppEngine().getAppById('dim_add');
                            if(addWindow.isOpen()){
                                addWindow.close();
                            }else {
                                addWindow.open();
                            }
                            this.fireEvent("add",this.activeNode);
                        }
                      },{
                        text:"批量添加",
                        scope:this
        ...
        InfoArea.superclass.constructor.call(this, config);
        this.addEvents([
            /**
             * @event change
             * 表格内容变化时触发.
